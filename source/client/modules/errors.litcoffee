...
Implement this if you need to transmit more than just the key and the message.
Shrub uses the result from this function to serialize the error over the wire.
...
Look up the error type and use it. If it's not registered, fall back to the
TransmittableError superclass.
...
Not an instance of `Error`. This probably shouldn't happen, but we deal with it
anyway.
