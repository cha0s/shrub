...
Registrar function passed to packages to allow them to register hooks and/or
...
* (optional string) `submodule` - Register this hook under a submodule. This is
  a convenience for when you might need to register more than one
  implementation of a hook, bur prefer to keep both implementations in the same
  physical file.

* (string) `hook` - The name of the hook to register.

* (function) `impl` - The hook implementation function. This is invoked when
  the hook is invoked. The signature of the implementation function may vary,
  consult the documentation for the specific hook to learn more.

###### TODO: Link to an instance of using the `submodule` arg in core if one exists.
...
*Invoke a hook with arguments. Return the result as an object, keyed by package
path.*
